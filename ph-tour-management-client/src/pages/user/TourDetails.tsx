...
export default function TourDetails() {
  ...
  const { data, isLoading } = useGetSingleTourQuery({ slug: slug as string });
  ...
  const tour = data;
  const division = data?.division as iDivisionResponse;
  const tourType = data?.tourType as iTourTypeResponse;
        ...
        <div>
          ...
          <div className="space-y-2">
            <p>
              ...
              -{" "}
              {format(
                new Date(tour?.endDate ? tour?.endDate : new Date()),
                "PP"
              )}
            </p>
            <p>
      ...
      <div className="mb-8">
        ...
        <ol className="space-y-3">
          {tour?.tourPlane?.map((plan, index) => (
            <li
              key={index}
              className="flex items-center border rounded-full p-1.5"
            >
